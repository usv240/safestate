      ...
      <Container className="py-12">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          <Card className="overflow-hidden p-0">
            ...
            <div className="border-t border-slate-100 px-5 py-2 text-xs font-medium text-ink-500">Response</div>
          ...
          <div className="space-y-6">
            ...
            <Card className="p-6">
              <h3 className="font-semibold text-ink-900">The contract</h3>
...
function Header({ icon, title, tone }: { icon: React.ReactNode; title: string; tone: "brand" | "slate" }) {
  return (
    <div className="flex items-center gap-2 border-b border-slate-100 px-5 py-3">
      <span className={`inline-flex h-7 w-7 items-center justify-center rounded-lg ${tone === "brand" ? "bg-brand-50 text-brand-700" : "bg-slate-100 text-ink-700"}`}>
        {icon}
      </span>
  ...
  return (
    <li className="flex gap-3">
      ...
      <span className="text-ink-500">
        <b className="text-ink-900">{title}.</b> {children}
      </span>
    </li>
  );
}
